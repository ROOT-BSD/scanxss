...
typedef struct {
    ...
    int        field_count;
    long       baseline_len;   /* кешований розмір чистої відповіді, -1 = не виміряно */
    bool       is_binary;      /* Content-Type не-HTML — не атакуємо */
} Form;
...
typedef struct {
    ...
    int         threads;             /* worker threads (0 = auto = 4)      */
} ScanConfig;
...
const char **payloads_get       (PlModule m);   /* NULL-terminated масив */
const char **payloads_markers   (PlModule m);
const char **payloads_hints     (PlModule m);
int          payloads_count     (PlModule m);
int          payloads_add       (const char *module, const char *payload,
                                 const char *source);
...
/* ─── HTTP Multi callback ─────────────────────────────────── */
typedef void (*HttpMultiCb)(HttpResponse *resp, const char *url,
...
char   *db_get_report_path  (ScanContext *ctx, int64_t scan_id); /* caller free() */
