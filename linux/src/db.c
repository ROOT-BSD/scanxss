...
static const char *SCHEMA =
    ...
    "  html_path   TEXT    DEFAULT ''"                 /* шлях до HTML звіту */
    ");"
...
/* ── Міграція: додаємо html_path якщо БД вже існує без цього поля ── */
static void db_migrate(ScanContext *ctx) {
    /* ALTER TABLE ігнорується якщо колонка вже є (SQLITE_ERROR) */
    sqlite3_exec(H(ctx),
        "ALTER TABLE scans ADD COLUMN html_path TEXT DEFAULT ''",
        NULL, NULL, NULL);
}

/* ── Зберігаємо шлях до HTML звіту по scan_id ─────────────── */
void db_save_report_path(ScanContext *ctx, const char *html_path) {
    if (!ctx->db || !ctx->scan_id || !html_path) return;
    sqlite3_stmt *s;
    sqlite3_prepare_v2(H(ctx),
        "UPDATE scans SET html_path=? WHERE id=?", -1, &s, NULL);
    sqlite3_bind_text (s, 1, html_path, -1, SQLITE_STATIC);
    sqlite3_bind_int64(s, 2, ctx->scan_id);
    sqlite3_step(s);
    sqlite3_finalize(s);
}

/* ── Отримуємо шлях до HTML звіту по scan_id ──────────────── */
char *db_get_report_path(ScanContext *ctx, int64_t scan_id) {
    if (!ctx->db) return NULL;
    sqlite3_stmt *s;
    sqlite3_prepare_v2(H(ctx),
        "SELECT html_path FROM scans WHERE id=?", -1, &s, NULL);
    sqlite3_bind_int64(s, 1, scan_id);
    char *path = NULL;
    if (sqlite3_step(s) == SQLITE_ROW) {
        const char *p = (const char *)sqlite3_column_text(s, 0);
        if (p && p[0]) path = strdup(p);
    }
    sqlite3_finalize(s);
...
/* ── build DB file path ────────────────────────────────────── */
/* Get ~/.scanxss base path */
static void get_home_scanxss(char *out, size_t sz)
    __attribute__((unused));
static void get_home_scanxss(char *out, size_t sz) {
    const char *home = getenv("HOME");
    if (!home) home = "/tmp";
    snprintf(out, sz, "%s/.scanxss", home);
}
...
#ifdef __APPLE__
    /* macOS: always use ~/.scanxss/scan.db */
    char home_dir[512] = {0};
    get_home_scanxss(home_dir, sizeof(home_dir));
    snprintf(out, out_size, "%s/scan.db", home_dir);
    return;
#endif

    /* Linux/BSD: --session-dir > exe_dir/../DB_SCAN */
...
int db_open(ScanContext *ctx) {
...
#ifdef __APPLE__
            get_home_scanxss(db_dir, sizeof(db_dir));
#else
    ...
    db_migrate(ctx);   /* додає нові колонки у існуючі БД */
...
int db_load_retarget_forms(ScanContext *ctx, int64_t scan_id) {
    ...
    /* Спочатку пробуємо точний JOIN: форми з вразливостями */
    sqlite3_prepare_v2(H(ctx),
        "SELECT DISTINCT f.url, f.method, f.fields"
        " FROM forms f"
        " JOIN findings fi ON fi.url=f.url"   /* JOIN по URL, не scan_id */
        " WHERE f.scan_id=? AND fi.scan_id=?",
        -1, &s, NULL);
    sqlite3_bind_int64(s, 1, src);
    sqlite3_bind_int64(s, 2, src);
    ...
    /* Якщо JOIN не дав результату — завантажуємо всі форми сканування.
     * Це трапляється коли форми і знахідки зберігались з різними scan_id
     * (наприклад при --resume або після міграції БД).                    */
    if (loaded == 0) {
        printf("[DB] Retarget: JOIN не знайшов форм, завантажую всі форми scan #%lld\n",
               (long long)src);
        sqlite3_prepare_v2(H(ctx),
            "SELECT url, method, fields FROM forms WHERE scan_id=?",
            -1, &s, NULL);
        sqlite3_bind_int64(s, 1, src);
        while (sqlite3_step(s)==SQLITE_ROW && ctx->crawl.form_count < MAX_FORMS) {
            Form *f = &ctx->crawl.forms[ctx->crawl.form_count++];
    ...
    printf("[DB] Retarget: завантажено %d форм зі scan #%lld\n",
           loaded, (long long)src);
    return loaded;
}
