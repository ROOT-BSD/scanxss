...
static void print_usage(const char *prog) {
    ...
    printf("База payload-ів:\n"
           "  --update            Оновити payload-и з мережі (PATT, SecLists, NVD)\n"
           "  --update-source S   Джерело: patt | seclists | nvd (за замовч. всі)\n"
           "  --payloads-db FILE  Шлях до payloads.db (за замовч. ~/.scanxss/payloads.db)\n"
           "  --payloads-stats    Показати статистику БД payload-ів\n\n");
    printf("Продуктивність:\n"
           "  --threads N         Потоків для фази атак (за замовч. 4, макс. %d)\n\n",
           WORKER_MAX_THREADS);
    printf("Інше:\n"
...
static void print_summary(const ScanContext *ctx) {
    double elapsed = progress_scan_elapsed();
    double rps = elapsed > 0 ? ctx->requests_made / elapsed : 0;

    /* Форматуємо тривалість як Xгод Xхв Xс */
    int h = (int)elapsed / 3600;
    int m = ((int)elapsed % 3600) / 60;
    int s = (int)elapsed % 60;
    char dur_fmt[32];
    if (h > 0)
        snprintf(dur_fmt, sizeof(dur_fmt), "%dгод %dхв %dс", h, m, s);
    else if (m > 0)
        snprintf(dur_fmt, sizeof(dur_fmt), "%dхв %dс", m, s);
    else
        snprintf(dur_fmt, sizeof(dur_fmt), "%dс", s);
    ...
    /* ── Тривалість окремим рядком знизу ── */
    if (ctx->config.color)
        printf("\n  " COL_BOLD "Тривалість:" COL_RESET
               "  " COL_CYAN "%s" COL_RESET
               "  (" COL_YELLOW "%.1f req/s" COL_RESET
               "  |  %d req  |  %.0f сек)\n\n",
               dur_fmt, rps, ctx->requests_made, elapsed);
    else
        printf("\n  Тривалість:  %s  (%.1f req/s | %d req | %.0f сек)\n\n",
               dur_fmt, rps, ctx->requests_made, elapsed);
}
...
static struct option long_opts[] = {
    ...
    {"get-report",   required_argument,0,OPT_GET_REPORT},
    ...
    {"update-source",required_argument,0,OPT_UPDATE_SOURCE},
    {"payloads-db",  required_argument,0,OPT_PAYLOADS_DB},
    {"payloads-stats",no_argument,     0,OPT_PAYLOADS_STATS},
...
int main(int argc, char *argv[]) {
    ...
    /* parse CLI */
    int64_t show_scan_id = 0, delete_scan_id = 0, get_report_id = 0;
    bool do_list = false, do_show = false, do_delete = false, do_wipe = false;
    bool do_get_report = false;
    ...
    for (int _j=1;_j<argc;_j++) {
        if (strcmp(argv[_j],"--email-history")==0 ||
            ...
            strcmp(argv[_j],"--list-modules")==0 ||
            strcmp(argv[_j],"--update")==0 ||
            strcmp(argv[_j],"--payloads-stats")==0) { _standalone=true; break; }
    }
    ...
    while ((opt=getopt_long(argc,argv,"u:d:t:l:s:r:m:o:f:vp:c:a:hV",
        ...
        case OPT_GET_REPORT:  do_get_report = true; get_report_id  = atoll(optarg); break;
        ...
        case OPT_SETUP_EMAIL:   break;
        case OPT_EMAIL_HISTORY: break;
        case OPT_UPDATE:        break; /* handled after getopt */
        case OPT_UPDATE_SOURCE: break; /* handled after getopt */
        case OPT_PAYLOADS_DB:
            payloads_set_db_path(optarg); break;
        case OPT_PAYLOADS_STATS: break;
        case OPT_THREADS:
            cfg->threads = atoi(optarg);
            if (cfg->threads < 1) cfg->threads = 1;
            if (cfg->threads > WORKER_MAX_THREADS) cfg->threads = WORKER_MAX_THREADS;
            break; /* handled after getopt */
    ...
    /* ── Ініціалізація бази payload-ів ── */
    payloads_init(cfg->exe_dir);

    /* open DB first for all operations */
    if (db_open(ctx) < 0) { payloads_close(); free(ctx); return 1; }
    ...
    if (do_get_report) {
        /* Виводить тільки шлях до HTML файлу — для парсингу GUI */
        char *path = db_get_report_path(ctx, get_report_id);
        if (path && path[0]) { printf("%s\n", path); free(path); }
    ...
    for (int _i=1; _i<argc; _i++) {
        if (strcmp(argv[_i], "--payloads-stats") == 0) {
            payloads_print_stats();
            db_close(ctx); payloads_close(); free(ctx); return 0;
        }
    }

    /* ── --update [--update-source SRC] ── */
    for (int _i=1; _i<argc; _i++) {
        if (strcmp(argv[_i], "--update") == 0) {
            const char *src = NULL;
            for (int _j=1; _j<argc-1; _j++)
                if (strcmp(argv[_j],"--update-source")==0) { src=argv[_j+1]; break; }
            update_payloads(cfg, src);
            db_close(ctx); payloads_close(); free(ctx); return 0;
        }
    }
    ...
    /* Розраховуємо threads для виводу так само як у attack.c */
    int show_threads;
    if (cfg->threads > 0) {
        show_threads = cfg->threads;
    } else {
        long ncpu = sysconf(_SC_NPROCESSORS_ONLN);
        if (ncpu < 1) ncpu = 1;
        show_threads = (int)(ncpu * 4);
        if (show_threads > 32) show_threads = 32;
        if (show_threads < 4)  show_threads = 4;
    }
    if (show_threads > WORKER_MAX_THREADS) show_threads = WORKER_MAX_THREADS;
    const char *threads_auto = cfg->threads > 0 ? "" : " (auto)";

    printf("Mode:   %s%s%s  |  depth:%d  timeout:%ds  rate:%d/s  threads:%d%s\n",
           COL_BOLD, mode_name[cfg->scan_mode], COL_RESET,
           cfg->depth, cfg->timeout, cfg->rate, show_threads, threads_auto);

    rate_init(&ctx->rate, cfg->rate);
    progress_global_reset();          /* єдиний таймер для всього сканування */
...
static void smtp_setup_wizard(ScanXSSConfig *cfg) {
    ...
    if (home) {
        ...
        if (cf) {
            fprintf(cf, "# ScanXSS v1.3.3 — автоматично збережений конфіг\n");
